...
```{r}
loadTraces <- function(directory_name, layersAll = c(""), tiernameAll = c(""), categoriesAll = list(c())){
  ...
  for (i in 1:length(categoriesAll)){
    if (length(tiernameAll) < length(categoriesAll)){
      tiernameAll <- append(tiernameAll, "")
    }
    if (length(layersAll) < length(categoriesAll)){
      layersAll <- append(layersAll, "")
    }
  }
  ...
  for (item in 1:length(categoriesAll)){
    ...
    if (tiernameAll[[item]] == "" && length(categoriesAll[[item]]!=0) && categoriesAll[[item]] == ""){
...
#categories <- c("nʲ", "n")
#append(list(c("")), c())[[2]]
myXY_data <- loadTraces(directory_name = "/Users/yayaorigami/Documents/plinguistics/updatedRussianPalatals/ultrasound-data-RussianPalatals-202012", layersAll = c('palate', 'tongue'), categoriesAll = list(c(""), c("n")))
myXY_data
...
```{r}
polarTraces <- makeTracesPolar(myXY_data = myXY_data, scaling.factor = 592/412, origin.algorithm = "BottomMean")
plotTraces(myXY_data = myXY_data, compiledList = polarTraces, points.display = FALSE, bands.lines = TRUE, bands.fill = TRUE, transparency = 0.37, mean.lines = TRUE, legend.position = "topleft", plot.labels = TRUE, legend.size = 1.5, legend.linewidth = 1.5, means.linewidth = 1.5, tick.size = 1.2, bands.linewidth = .3, plot.ticks = TRUE)
```
