...
plotStyleTraces <- function(rawTraces, matrixIntersection, polarTraces, dataOfEachCurveNNj, uniqueSegments, palette = c(),
  ...
  if (h == 0){
    ...
    x <- (b2 - averaged_everything[[3]]) / (averaged_everything[[1]] - perp_l)
    y <- averaged_everything[[1]] * x +  averaged_everything[[3]]
    
    center_l = y_xl + (end_xl - start_xl) * averaged_everything[[1]]
    
    start_xm = min(df[, 5])
    end_xm = max(df[, 12])
    y_xm = df[which.max(df[, 12]), 14]
    
    center_m = y_xm - (end_xm - start_xm) * averaged_everything[[2]]
  }
  
  if (h == 1){
    start_xl = max(df[, 11])
    end_xl = averaged_everything[[5]] #min(df[, 5])
    y_xl = df[which.max(df[, 11]), 13] #add in respective x coor
    
    center_l = y_xl + (end_xl - start_xl) * averaged_everything[[1]]
    
    start_xm = averaged_everything[[5]] #max(df[, 5])
    end_xm = min(df[, 12])
    y_xm = df[which.max(df[, 12]), 14]
  ...
  if (parallelRays){
    ...
    segments(start_xl, y_xl, end_xl, center_l, col = "black", lwd = 2, lty = 2)
    segments(start_xm, center_m, end_xm, y_xm, col = "black", lwd = 2, lty = 2)
  }
...
#add 
#put into pdf
findX2 <- plotTraces(rawTraces, categoriesAll = list(c('ɫ.#_V.F', 'ɫ.V_#.F', 'ɫ.V_V.FF', 'ɫ.#_V.B', 'ɫ.V_#.B', 'ɫ.V_V.BB'), c('nˠ.#_V.B', 'nˠ.V_#.B', 'nˠ.V_V.BB', 'nˠ.#_V.F', 'nˠ.V_#.F', 'nˠ.V_V.FF'),  c('sˠ.#_V.F', 'sˠ.V_#.F', 'sˠ.V_V.FF', 'sˠ.#_V.B', 'sˠ.V_#.B', 'sˠ.V_V.BB', 'zˠ.V_#.F', 'zˠ.V_#.M', 'zˠ.V_V.MM', 'zˠ.V_#.B'), c('mˠ.#_V.F', 'mˠ.V_#.F', 'mˠ.V_V.FF', 'mˠ.#_V.B', 'mˠ.V_#.B', 'mˠ.V_V.BB'), c('kˠ.#_V.F', 'gˠ.#_V.F', 'kˠ.#_V.M', 'gˠ.V_#.B', 'gˠ.V_V.BB', 'kˠ.#_V.B')), mergeCategories = TRUE, maskCategories = c("ɫ", "nˠ", "Sˠ", "mˠ", "Kˠ"), legend.size = 1, parallelRays = TRUE, perpendicularRays = TRUE, h = 0)
