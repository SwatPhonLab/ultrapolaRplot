...
loadAllTracesMidPoint <- function(directory_name){
  ...
  for (item in 1:length(metaData$traces)){
          ...
          for(frame in 1:length(df$frame)){
              ...
              if (length(xyFileData) > 0) {
                ...
                for (smallerInterval in 1:length(filteringAllRowsTextGrids$xmax)){
                   ...
                   #overlapping <- rbind(overlapping, c(textOnTier, plainTextName, frameNumber, annotated))
                ...
                df$overlappingSegments[[frame]] <- overlapping
                df$overlappingTiers[[frame]] <- additionalTiers
              }else{
            ...
            #added some additional columns to df with overlapping tiers and boolean if they are annotated
            
            #can already expand df?
            #find all unique tiers
            allTiers <- na.omit(unlist(unique(df$overlappingTiers)))
            #for each row add a list of same length with the column names.
            #entries are the text or NA if doesn't exist
            #the row entry of [1] works for all rows..
            if (length(allTiers) > 0){
              for (tier in 1:length(allTiers)){
                df[[(allTiers[[tier]])]][1] <- NA
              }
            }
            
            #instead of NA fill up with text
            for (midpoint in 1:length(df$mid_point)){
              overlappingTiersRow <- na.omit(df$overlappingTiers[[midpoint]])
              overlappingTextRow <- na.omit(df$overlappingSegments[[midpoint]])
              if (length(overlappingTiersRow) > 0){
                for (overlappingTier in 1:length(overlappingTiersRow)){
                  df[[overlappingTiersRow[[overlappingTier]]]][[midpoint]] <-overlappingTextRow[[overlappingTier]]
                }
              }
            }
            
            #no need for the overlappingTier or Segments columns -- has been expanded to individual columns
            df <- df %>% select(-overlappingTiers, -overlappingSegments)
          
            #can still keep unnannoted segments by removing if statement
            for (midpoint in 1:length(df$mid_point)){
              if (df$annotated[[midpoint]] == 1){ #will at least overlap with itself
                allRowsTextGrids <- rbind(allRowsTextGrids, data.frame(df[midpoint,])) #necessary in case multiple text grid files
              }
            }
    ...
    if (!is.null(colnames(allRowsTextGrids))){
      allTiersLabelling <- colnames(allRowsTextGrids)
    }
    #return(allRowsTextGrids)
    #extract xy data separately once we have all the data
    #filter by layered tiers
      ...
      for(frame in 1:length(allRowsTextGrids$frame)){
        ...
        if (length(xyFileData)>0){
          for (mark in 1:length(xyFileData)){
            ...
            #build data.frame
            allFrame <- c(unlist(allRowsTextGrids[frame, all()]), itemNumber, xCoor, yCoor)
  ...
  colnames(rawTraces) <- append(allTiersLabelling, c("Item Number", "X", "Y"))
  rownames(rawTraces) <- NULL
  #column rearrangment and renaming to match with rest of functions
  #rawTraces <- subset(rawTraces, select = c())
...
myXY_data <- loadAllTracesMidPoint(directory_name = "/Users/yayaorigami/Documents/plinguistics/Dataset3")#, categoriesAll = c("u", "y"))
myXY_data2 <- loadAllTraces(directory_name = "/Users/yayaorigami/Documents/plinguistics/Dataset3")
