...
filteredTraces <- plotTraces(rawTraces, mergeCategories = c(TRUE), tiernameAll = c("vow", "vow", "vow", "vow", "vow", "vow", "vow", "vow"), categoriesAll = list(c('i', 'iː'), c('y', 'yː'), c('u'), c('ɯ', 'ɯː'), c('ɛ'), c('œ'), c('a'), c('ɔ')),  maskCategories = c("i/iː", "y/yː", "u", "ɯ/ɯː", "ɛ", "œ", "ɑ", "ɔ"), palette = c(LightGreen, DarkGreen, LightGreen, Pink, DarkBlue, DarkBlue, DarkRed, LightRed), means.styles = c(1,2,1,2,1,2,1,2), legend.size=0.8, legend.position="topleft", legend.linewidth=0.5, percentage_back = c(0.35), angle_pos = c(10), parallelRays = TRUE, perpendicularRays = TRUE, h = 0)


# plotTraces(rawTraces, categoriesAll = list(c('a'), c('ʊ'), c('u'), c('ʏ'), c('ə'), c('ɘ')), legend.size = 1.0, legend.position = "bottomright", perpendicularRays = TRUE, parallelRays = TRUE, h = 1, percentage_front = c(0.1, 0.3, 0.5, 0.6, 0.7, 0.9), percentage_back = c(0.1, 0.3, 0.5, 0.8), points.display = TRUE) #c('i'),c('y')
...
pairwise_comparison <- function(filteredTraces, interval = 1, singleIncrements = TRUE,  origin.algorithm = "BottomMiddle", origin.x = NA,
  ...
  comparisons_plot = compare_means(Values ~ Groups, data = data2, method = "t.test", p.adjust.method = "bonferroni", var.equal = FALSE)
