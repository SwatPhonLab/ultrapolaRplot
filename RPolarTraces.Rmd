...
plotStyleTraces <- function(rawTraces, matrixIntersection, polarTraces, dataOfEachCurveNNj, uniqueSegments, palette = c(),
  ...
  #SSANOVA and GAMS
    df_ssanova <- do.call(rbind, lapply(names(polarTraces), function(cat) {
    do.call(rbind, lapply(seq_along(polarTraces[[cat]]), function(i) {
      mat <- polarTraces[[cat]][[i]]
      ...
      data.frame(
        ...
        x     = as.numeric(mat[1, ]),
        y     = as.numeric(mat[2, ]),
        theta = as.numeric(mat[3, ]),
        r     = as.numeric(mat[4, ])
      )
    }))
  }))
  
  df_ssanova <- unique(df_ssanova)
  
  # Preserve the original order from uniqueSegments
  df_ssanova$category <- factor(df_ssanova$category, levels = uniqueSegments)
  cats <- uniqueSegments
  
  # Make sure colors and line styles match category names, not position
  names(paletteColors) <- uniqueSegments
  names(means.styles) <- uniqueSegments
  
  pred_df <- do.call(rbind, lapply(cats, function(cat) {
    df_cat <- subset(df_ssanova, category == cat)
  
    fit <- ssanova(r ~ theta, data = df_cat)
  
    theta_new <- seq(
      min(df_cat$theta, na.rm = TRUE),
      max(df_cat$theta, na.rm = TRUE),
      length.out = 300
    )
  
    newdat <- data.frame(theta = theta_new)
    ...
    data.frame(
      ...
      theta    = theta_new,
      r_fit    = pred$fit,
      r_lo     = pred$fit - 1.96 * pred$se.fit,
      r_hi     = pred$fit + 1.96 * pred$se.fit
    )
  }))
  
  # Keep category order in prediction data
  pred_df$category <- factor(pred_df$category, levels = uniqueSegments)
  
  # Convert predictions to Cartesian coordinates
  pred_df$x_fit <- pred_df$r_fit * cos(pred_df$theta)
  pred_df$y_fit <- pred_df$r_fit * sin(pred_df$theta)
  
  pred_df$x_lo <- pred_df$r_lo * cos(pred_df$theta)
  pred_df$y_lo <- pred_df$r_lo * sin(pred_df$theta)
  
  pred_df$x_hi <- pred_df$r_hi * cos(pred_df$theta)
  pred_df$y_hi <- pred_df$r_hi * sin(pred_df$theta)
  
  # Build closed radial confidence bands
  band_df <- do.call(rbind, lapply(cats, function(cat) {
    d <- subset(pred_df, category == cat)
  
    data.frame(
      category = cat,
      x = c(d$x_hi, rev(d$x_lo)),
      y = c(d$y_hi, rev(d$y_lo))
    )
  }))
  
  # Keep category order in band data
  band_df$category <- factor(band_df$category, levels = uniqueSegments)
    ...
    ggplot() +
      geom_path(
        ...
        aes(
          ...
          y = y,
          color = category,
          group = interaction(category, trace_id)
        ),
        alpha = 0.00,
        linewidth = 0.4
      ) +
      ...
      scale_color_manual(values = paletteColors, breaks = uniqueSegments) +
      scale_fill_manual(values = paletteColors, breaks = uniqueSegments) +
      scale_linetype_manual(values = means.styles, breaks = uniqueSegments) +
      coord_fixed() +
      theme_minimal()
  )
  #END
...
#rawTracesEx <- loadAllTracesMidPoint("~/Documents/SwatSpring2026/CRAN Submission/ultrapolaRplot_0_2_1/inst/extdata")
```

```{r}
# filteredTraces <- plotTraces(rawTracesEx,
# categoriesAll = c("o", "i"), bestFitRays = TRUE,
# bestFitRays.intersection_rays.negative = c(0.41), bestFitRays.intersection_rays.positive = c(0.2), difference_plot = TRUE, identify_max_negative = TRUE, identify_max_positive = TRUE)
DarkRed = '#B30000'
#LightRed = '#FF69B5' 
LightRed = '#FF6767' 
DarkBlue = '#0000FF'
...
means.styles = c(1,2,1,2,1,2,1,2), legend.size=2, legend.position="bottomleft", legend.linewidth=2, bestFitRays.intersection_rays.negative = c(0.48), bestFitRays = TRUE)

# filteredTraces <- plotTraces(rawTracesD, mergeCategories = c(FALSE), tiernameAll = c("vow", "vow"),
#                              categoriesAll = list(c('i', 'iː', 'y', 'yː', 'ɛ', 'œ'), c('ɯ', 'ɯː', 'u', 'a', 'ɔ')),
#                              maskCategories = c("front vowels", "back vowels"),
#                              means.styles = c(1,1), legend.size=2, legend.position="bottomleft", legend.linewidth=2, difference_plot = TRUE, bestFitRays = TRUE, bestFitRays.intersection_rays.negative = c(0.47))
...
```{r}
#basic_pairwise(rawTraces, x_coor = x_max, y_coor = y_target_l, angle = neg_perc, origin.algorithm = origin.algorithm)
#neg_perc * pi / 180
