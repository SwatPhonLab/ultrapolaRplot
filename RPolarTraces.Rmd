...
myXY_data <- loadTraces(directory_name = "/Users/yayaorigami/Documents/plinguistics/Dataset3", tiername = "vowels", categories <- c('y'))
myXY_data
```
```{r}
unique(myXY_data$file_number)
```

```{r}
polarTraces <- makeTracesPolar(myXY_data = myXY_data, scaling.factor = 592/412, origin.algorithm = "BottomMean")
plotTraces(myXY_data = myXY_data, compiledList = polarTraces, points.display = TRUE, bands.lines = TRUE, bands.fill = TRUE, transparency = 0.37, mean.lines = TRUE, means.styles =  c(2, 1, 1, 1, 1, 2, 1, 2), legend.position = "topleft", plot.labels = TRUE, legend.size = 1.2, legend.linewidth = 1.5, means.linewidth = 1.5, tick.size = 1.2, bands.linewidth = .3, palette = c(
  
  "#FF3366"))#, png.filename = "ok4.png")
```
