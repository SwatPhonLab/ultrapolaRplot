            ...
            for (fileTrace in 1:length(cur_recording)){
              if (length(cur_recording[[fileTrace]])>0){
                ...
                for (mark in 1:length(filenamexy)){
                  ...
                  #lack of segment and tiers
                  appendedFrame <- data.frame(myFileAndFrameName, mark, I(list(NA)), xCoor, yCoor, I(list(NA)), layer)
                  colnames(appendedFrame) <- c('file_number', 'itemNumber_inFile', 'segment_list', 'x', 'y', 'tiers_list', 'layer')
                  rawTraces <<- rbind(rawTraces, appendedFrame)
                  #layer as segment name, used for plotting.
                }
                filenamexy <- list()
      ...
      for(frame in 1:length(allRowsTextGrids$frame)){
        ...
        if (length(xyFileData)>0){
          for (mark in 1:length(xyFileData)){
            ...
            allFrame <- data.frame(myFileAndFrameName, itemNumber, I(segments), xCoor, yCoor, I(tiers), layerOf)
            colnames(allFrame) <- c('file_number', 'itemNumber_inFile', 'segment_list', 'x', 'y', 'tiers_list', 'layer')
...
```{r}
test <- myXY_data[sapply(myXY_data$segment_list, function(x) any(c("n") %in% x)),]
expanded_data <- myXY_data %>% unnest(c(segment_list, tiers_list))
expanded_data
